...
### 27/08/2026 — proposta de painel lateral para Normas técnicas

- o novo padrão visual ficou restrito ao cadastro de Normas técnicas enquanto aguarda aprovação; a alteração global de composição dos demais formulários especializados foi desfeita para não propagar uma solução ainda não validada pela pessoa usuária;
- no desktop, a ficha normativa é um painel lateral encostado no topo, na direita e na base da área de trabalho, com entrada da direita para a esquerda. Em movimento reduzido, a animação é removida;
- o cabeçalho abandonou o bloco escuro e separa claramente contexto, modo, título e descrição em uma superfície branca minimalista. No mobile, a mesma hierarquia é preservada e o texto redundante “Cadastro especializado” foi removido apenas desta ficha;
- os botões `!` têm indicador visual de 16 px, contraste baixo e painel explicativo neutro, mantendo uma área clicável real de 44 px para toque e teclado;
- título publicado, emenda/errata e referência oficial permanecem como campos largos, enquanto identificação, edição e assunto aproveitam a linha no modo essencial. IDs, payload, permissões e validações do servidor não foram alterados;
- cache PWA atualizado para `sivs-v2.2.0-norms-drawer-102`. A auditoria rápida em 390 px aprovou três telas e onze interações, incluindo ajuda contextual, sem overflow ou falha de toque.

### 27/08/2026 — liberação progressiva dos detalhes do cadastro

- todos os cadastros especializados agora mantêm os campos complementares recolhidos em registros novos e os liberam automaticamente uma única vez quando os campos obrigatórios forem concluídos;
- depois de liberados, se a pessoa recolher os detalhes manualmente, essa escolha é preservada. O sistema não reabre o painel a cada alteração posterior;
- a linguagem do bloco foi simplificada para “Dados obrigatórios”, “Detalhes complementares” e “Ver detalhes”, com instrução objetiva sobre a liberação automática. O marcador decorativo da ficha de Normas técnicas foi reduzido a um ponto neutro;
- a auditoria responsiva passou a preencher todos os controles obrigatórios da ficha de Normas técnicas e a exigir abertura automática e visibilidade dos complementos. Em 390 px, três telas e doze interações foram aprovadas sem overflow ou falhas de toque.

### 27/08/2026 — NF-e 4.00 assinada e autorizada em homologação

- foi incorporado o pacote XSD oficial NF-e `010e_v1.02`, publicado em 10/07/2026, preservado com fonte e SHA-256 `D44AE5AA6A0D1CABF6235D2D2D47B75BE5DD87BC6B90A7EC3DCEC99C3D41BDA1`; `tools/sync_nfe_schemas.py` verifica tamanho, hash e lista permitida e só grava com `--apply`;
- o novo núcleo `fiscal_nfe.py` forma e confere a chave de 44 dígitos, gera XML NF-e 4.00 determinístico a partir da fotografia fiscal, assina `infNFe` com XMLDSig RSA-SHA1 exigido pelo leiaute, verifica digest/assinatura e valida o XML assinado no XSD sem rede ou entidades externas;
- série e número são reservados atomicamente por empresa, unidade, ambiente e modelo, sem reutilização nem colisão concorrente. Falha de preparo ou rede permite repetir exatamente a mesma numeração e data já reservadas, sem avançar outra vez a sequência;
- a transmissão usa endpoint oficial configurado, SOAP 1.2, TLS 1.2+ e o mesmo A1 criptografado já isolado por empresa/unidade. A rota é exclusiva de homologação, exige `issue_nfe_homologation`, revisão otimista e confirmação literal `HOMOLOGAR`; produção segue bloqueada;
- somente cStat 100/150 com protocolo para a mesma chave promove o documento a `AUTHORIZED`. XML assinado, requisição, resposta e `nfeProc` são preservados com SHA-256; o XML processado pode ser baixado e o DANFE é gerado com código de barras, protocolo e marca d’água **Homologação — sem valor fiscal**;
- a configuração passou a coletar endereço fiscal completo; o cadastro unificado de parceiros recebeu número, complemento, código IBGE, indicador de IE e inscrição estadual. A tela lista rejeição/motivo, permite emissão de rascunho e expõe XML/DANFE somente após autorização;
- limite consciente: o gerador bloqueia CST/CSOSN sem grupo XML homologado em vez de inventar campos. Cancelamento, CC-e, inutilização, contingência, produção e os grupos IBS/CBS dependentes da classificação tributária da empresa continuam fora da liberação atual e devem ser implementados/homologados antes de dizer que “só falta o A1”.
- validações executadas: assinatura e adulteração, XSD oficial, vínculo protocolo/chave, `nfeProc`, DANFE PDF, sintaxe Python/JavaScript e 166 testes; incompatibilidades de contrato sobre a semeadura do schema e a orientação antiga do rascunho foram corrigidas, mantendo o banco vazio até a configuração fiscal e distinguindo rascunho de emissão em homologação.

### 27/08/2026 — ambiente operacional de RH, ponto e folha

- o antigo cadastro simples de colaboradores foi preservado como mestre, e o novo módulo `rh` ganhou domínio próprio para vínculos trabalhistas, matrícula eSocial, unidade, salário em centavos, divisor mensal, jornada semanal, dependentes, situação e revisão; leitura, escrita, valores e exportação possuem permissões específicas, e operadores/visualizadores não recebem acesso de RH por padrão;
- marcações importadas são fatos imutáveis. O importador aceita AFD leiaute 004 em ISO-8859-1/CRLF, identifica REP-C, REP-A ou REP-P e confere cabeçalho, trailer, ordem e unicidade do NSR, CRC-16/KERMIT e cadeia SHA-256 do REP-P, ou CSV com CPF e data/hora. Arquivo duplicado é idempotente, CPF sem vínculo é relatado e CNPJ do AFD precisa corresponder à unidade selecionada;
- ajustes de ponto nunca editam o relógio: entram como nova marcação `MANUAL`, com autor, data, justificativa e hash. A apuração pareia entradas/saídas, mede jornada prevista, trabalhada, extra e ausência e bloqueia fechamento diante de quantidade ímpar, intervalo inválido ou ausência total de marcações;
- `hr_payroll.py` mantém cálculo determinístico para competências de 2026: INSS progressivo conforme Portaria Interministerial MPS/MF nº 13/2026, IRRF mensal e redução da Lei nº 15.270/2025 conforme tabela da Receita atualizada em 27/04/2026, FGTS de 8%, hora extra, falta e eventos avulsos com incidências explícitas. Competência sem tabela versionada é recusada;
- a folha possui prévia revisionada, fotografia do vínculo/jornada/cálculo, fechamento otimista e imutável, holerite PDF com marca d'água quando ainda for prévia, CSV contábil com bases e encargos, CSV de horas e AEJ leiaute 002 em ISO-8859-1. Todos os downloads registram SHA-256 e auditoria;
- a interface `hr-management.js` reúne vínculos, importação de relógio, ajustes, apuração, eventos, folha, holerite e exportações em fluxo responsivo, acessível por teclado e compatível com movimento reduzido. Cache PWA atualizado para `sivs-v2.2.0-hr-payroll-104`;
- fontes oficiais consultadas em 27/08/2026: página REP/Portaria 671 do MTE e leiautes AFD/AEJ atualizados em 31/07/2026; eSocial S-1.3 consolidado até NT 06/2026; tabela INSS 2026; tabela IRRF 2026. Validação final: 178 testes aprovados, além de compilação Python, sintaxe JavaScript e `git diff --check`;
- limite consciente: o AEJ gerado possui o leiaute de dados 002, mas ainda não possui o arquivo P7S assinado pelo desenvolvedor do PTRP e por isso é uma exportação de validação, não um artefato fiscal completo. O módulo também não transmite eSocial/DCTFWeb/FGTS Digital, não calcula férias, 13º, rescisão, adicionais noturnos/periculosidade/insalubridade, múltiplos vínculos ou regras de convenção coletiva; esses fluxos exigem implementação e homologação antes de tratar o RH como substituto integral de um sistema trabalhista certificado.

### 28/08/2026 — Central de relatórios componível e governada

- foi criado o módulo `relatorios` e o catálogo seguro `reporting.py`, com oito fontes: registros operacionais, financeiro, comercial, estoque, folha, licitações, contabilidade e auditoria. Cada fonte publica somente dimensões, métricas e expressões previamente revisadas; não existe SQL livre ou nome de coluna aceito diretamente do cliente;
- a pessoa pode combinar de uma a quatro dimensões e de uma a seis métricas, filtrar período, texto, módulos autorizados e até quatro dimensões por valores exatos, além de ordenar o resultado. A consulta usa parâmetros, sempre exige `company_id`, limita a apresentação a 500 agrupamentos e calcula totais gerais em consulta separada sobre todo o recorte;
- o catálogo é calculado para a sessão: exige leitura da Central e da fonte original; métricas sensíveis dependem da função de valores da fonte. Exportar exige exportação tanto em `relatorios` quanto na origem, e auditoria só aparece para quem possui essa capacidade. Um teste com operador restrito a Relatórios + Vendas comprovou ausência de Folha/Auditoria e bloqueio de exportação;
- a interface `reporting-center.js` oferece seleção visual de fonte, filtros por módulo e dimensão, indicadores, gráfico de barras, tabela com total geral, estados vazios, modelos pessoais e compartilhados, CSV UTF-8 e PDF paisagem com totais. Teclado, região viva, alertas, toque, movimento reduzido e larguras móveis foram preservados;
- modelos ficam em `report_templates`, isolados por empresa e proprietário; apenas admin/gestor pode compartilhar. Criação, execução, exclusão e exportação são auditadas, e downloads recebem SHA-256. Erros internos de banco são registrados no servidor e retornam mensagem genérica, sem expor detalhes do esquema;
- a Central é deliberadamente extensível, mas não promete consulta arbitrária: uma nova fonte ou campo exige inclusão revisada no catálogo. Isso protege desempenho, privacidade e consistência contábil. O cache PWA foi atualizado para `sivs-v2.2.0-reporting-center-105`;
- validação final: 185 testes automatizados aprovados em 106,47 s, compilação Python, sintaxe JavaScript e contratos de frontend. As oito fontes foram testadas individualmente com entradas e totais conhecidos, incluindo saldo após baixa, quantidades/descontos comerciais, saldo físico/reservado/disponível, folha, edital, partida dobrada e auditoria. O teste de volume insere 100 mil registros da empresa ativa e 20 mil de outra empresa: três consultas de agregação, detalhe limitado e pesquisa levaram 398,8 ms, com totais conferidos centavo a centavo por cálculo independente e sem vazamento multiempresa; oito leitores simultâneos, cada um apurando 100 mil registros, terminaram em 2,44 s com resultados idênticos. A auditoria visual percorreu desktop, tablet, 390 px e 360 px; após substituir o carrossel móvel que gerava overflow por cartões em coluna e rótulos quebráveis, as execuções finais de 58 telas e 12 interações em cada largura móvel terminaram sem overflow, controles comprimidos ou falhas de interação.

### 28/08/2026 — auditoria ponta a ponta após Fiscal, RH e Relatórios

- o laudo `sivs_2_2/AUDITORIA_PONTA_A_PONTA_2026-08-28.md` consolida a inspeção de 53 módulos, 455 ações, 119 caminhos literais de API, 99 tabelas, 101 gatilhos, 96 índices, 185 testes e 232 combinações visuais;
- a suíte integral e a simulação de 22 cenários passaram, o banco novo retornou `integrity_check=ok` sem violações de chave estrangeira e não foi reproduzido vazamento multiempresa, bypass de permissão ou divergência de totais;
- a aprovação permanece condicionada: `pypdf 5.9.0` é usado em PDFs externos do PNCP com `strict=False`, a faixa `<6` impede receber correções de indisponibilidade publicadas em 2026 e a extração deve migrar para versão corrigida e processo isolado;
- os perfis-base Operador e Consulta foram classificados como amplos demais para menor privilégio: o primeiro escreve em 45/53 módulos e o segundo lê 52/53, inclusive com valores em áreas sensíveis. O motor de permissões efetivas funciona, mas os presets precisam ser restritos por função empresarial;
...
### 29/08/2026 — automação empresarial supervisionada e busca de editais às 7h

- a varredura autônoma de licitações deixou a recorrência relativa de duas horas e passou a uma agenda local explícita: 7h em `America/Sao_Paulo`, de segunda a sábado. Domingo é recusado inclusive em retomada após indisponibilidade; uma agenda vencida no domingo é adiada para segunda às 7h;
- cada busca concluída materializa no máximo um resumo por empresa e data local, com o texto `X licitações encontradas hoje`, quantidade nova, orientação de revisão e vínculo auditável ao `tender_job`. Envio ao portal, lance, preço e documentos finais continuam fora da autonomia;
- `company_automation_settings`, `company_automation_runs` e `company_automation_findings` formam a camada comum para Financeiro, Compras, Estoque, Fiscal/contábil, RH e Qualidade/técnico. A chave única empresa+data impede duplicidade, e gatilhos recusam execução, evidência ou registro de outra empresa;
- o diagnóstico diário somente lê fatos persistidos e prepara achados: vencimentos, possíveis duplicidades, extratos sem conciliação e previsão de caixa de 30 dias pelo saldo aberto; solicitações de compra; estoque totalmente reservado e transferência possível; rascunhos/rejeições e produtos sem perfil fiscal; onboarding, conformidade, férias, prévias e prazos de RH; documentos técnicos e itens de qualidade em revisão. Ele não grava baixa, pagamento, conta bancária, pedido, movimento físico, documento fiscal, partida, folha ou resultado técnico;
- a IA externa é opcional e usada somente para redigir uma frase de priorização a partir de contagens agregadas por área e severidade. Títulos, pessoas, documentos, valores e evidências não saem nessa chamada; indisponibilidade produz resumo determinístico sem impedir a execução;
- a Controladoria ganhou uma Central de automação acessível por teclado e toque, com próxima execução, histórico, limites e achados filtrados pelos módulos legíveis da sessão. Ciência e resolução alteram apenas o estado do achado e registram auditoria;
