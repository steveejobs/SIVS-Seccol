(function tenderDocumentsModule() {
  ...
  function requirementRow(requirement, escape, editable, inversion) {
    ...
    return `<div class="tender-requirement ${custom ? "is-custom" : ""}" data-tender-requirement data-document-type="${escape(requirement.document_type)}" data-custom="${custom ? "1" : "0"}" data-title="${escape(requirement.title)}" data-portal-declaration="${portal ? "1" : "0"}">
      <label class="tender-requirement-check"><input type="checkbox" data-required ${requirement.required ? "checked" : ""} ${editable ? "" : "disabled"}><span><strong>${escape(requirement.title)}</strong><small>${escape(catalog.group || "Documento do edital")}${portal ? " · normalmente preenchida no portal" : ""}${requirement.extraction_suggestion ? " · sugerido pela extração; confirme no edital" : ""}</small></span></label>
  ...
  function detailHTML(data, tenderId, helpers) {
    ...
    const groupHTML = [...groups.entries()].map(([group, requirements]) => `<details class="tender-requirement-group" ${requirements.some((item) => item.required || item.extraction_suggestion) ? "open" : ""}><summary><strong>${escape(group)}</strong><span>${requirements.filter((item) => item.required).length} marcado(s)</span></summary>${requirements.map((item) => requirementRow(item, escape, editable, inversion)).join("")}</details>`).join("");
