(() => {
  ...
  function detailHTML(data, tenderId, helpers = {}) {
    ...
    return `<section class="tender-detail-section portal-agent" aria-labelledby="portalAgentTitle-${tenderId}">
      <div class="panel-head"><div><p class="eyebrow gold">AGENTE DE PORTAL</p><h3 id="portalAgentTitle-${tenderId}">Disputa governada pela proposta v${escape(policy.proposal_version)}</h3><small class="muted">A IA observa e sugere; o SIVS valida piso, passo, janela e duplicidade no servidor.</small></div><span class="status ${policy.status === "ARMED" ? "ativo" : "pendente"}">${escape(labels[policy.status] || policy.status)}</span></div>
