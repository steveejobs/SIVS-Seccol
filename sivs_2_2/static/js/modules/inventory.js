(function initializeInventory(global) {
  ...
  function balanceTable(data, escapeHTML) {
    if (!data.balances.length) {
      return '<div class="empty">Nenhum saldo registrado. Faça uma entrada conferida para iniciar o histórico de estoque.</div>';
    }
  ...
  function dialogs(data, escapeHTML) {
    ...
    return `
      ...
      <dialog id="inventoryMovementDialog" class="dialog form-drawer" aria-labelledby="inventoryMovementTitle"><form id="inventoryMovementForm"><div class="dialog-head"><div><p class="eyebrow gold">HISTÓRICO PROTEGIDO</p><h2 id="inventoryMovementTitle">Registrar movimento</h2></div><button type="button" class="icon-button" data-inventory-close aria-label="Fechar">×</button></div><p class="compliance-note compact">O saldo não é editado diretamente. Esta ação registra o motivo, atualiza quantidade e valor e preserva o histórico para conferência.</p><div class="inventory-form-grid"><label class="field"><span>O que aconteceu? *</span><select name="movementType" required>${movements}</select></label><label class="field"><span>Reserva que será usada</span><select name="reservationId"><option value="">Sem reserva</option>${reservationOptions}</select></label><label class="field"><span>Produto *</span><select name="productId" required><option value="">Selecione</option>${productOptions}</select></label><label class="field"><span>Depósito *</span><select name="warehouseId" required><option value="">Selecione</option>${warehouseOptions}</select></label><label class="field hidden" data-transfer-destination><span>Depósito de destino *</span><select name="counterpartWarehouseId"><option value="">Selecione</option>${warehouseOptions}</select></label><label class="field"><span>Lote</span><input name="lot" maxlength="120"></label><label class="field"><span>Quantidade *</span><input name="quantity" type="number" min="0.000001" step="0.000001" required></label><label class="field hidden" data-unit-cost><span>Custo unitário *</span><input name="unitCost" inputmode="decimal" placeholder="0,00"></label><label class="field"><span>Documento que gerou o movimento *</span><select name="originType" required><option value="">Selecione</option>${origins}</select></label><label class="field"><span>Código ou número do documento *</span><input name="originId" maxlength="120" required placeholder="Ex.: PC-2026-0042"></label><label class="field"><span>Referência adicional</span><input name="reference" maxlength="240"></label><label class="field full"><span>Por que este ajuste é necessário?</span><textarea name="reason" rows="3" maxlength="500"></textarea></label></div><div class="dialog-actions"><button type="button" class="secondary" data-inventory-close>Cancelar</button><button type="submit" class="primary">Registrar movimento</button></div></form></dialog>
  ...
  inventory.load = async function loadInventory({ api, state, writable, canAction, escapeHTML, dateBR, toast }) {
    ...
    content.innerHTML = `<section class="inventory-hero"><div><p class="eyebrow gold">ESTOQUE COM HISTÓRICO</p><h2>Estoque atualizado a cada movimentação</h2><p>Veja o que há fisicamente, o que já está reservado e o que ainda pode ser usado, por produto, lote e depósito.</p></div><div class="inventory-hero-actions">${abilities.warehouse ? '<button id="inventoryNewWarehouse" class="secondary" type="button">＋ Depósito</button>' : ""}${abilities.reserve ? '<button id="inventoryNewReservation" class="secondary" type="button">＋ Reserva</button>' : ""}${abilities.movement ? '<button id="inventoryNewMovement" class="primary" type="button">＋ Movimento</button>' : ""}</div></section>
      ${data.legacyRecordCount ? `<p class="compliance-note"><strong>Atenção:</strong> existem ${Number(data.legacyRecordCount)} registro(s) antigos, preservados apenas para consulta. Eles não alteram o saldo atual do estoque.</p>` : ""}
      ...
      <section class="panel inventory-history-panel"><div class="panel-head"><div><h3>Histórico de movimentações</h3><small class="muted">Mostra quem fez cada alteração, o documento relacionado e o efeito na quantidade${abilities.values ? " e no valor" : ""}. O histórico não pode ser alterado.</small></div><span class="status">Últimos ${data.movements.length}</span></div>${movementTable(data, escapeHTML, dateBR)}</section>
