(function initializeFiscalIntegration(global) {
  ...
  function fiscalDraftWorkspaceHTML({ taxSetup, drafts, branches, abilities, escapeHTML, currentDate }) {
    ...
    const draftItems = (drafts?.items || []).length ? `<div class="fiscal-draft-list">${drafts.items.map((item) => `<div><strong>Rascunho #${item.id} · ${escapeHTML(item.source_title)}</strong><small>${escapeHTML(item.branch_name)} · ${escapeHTML(item.operation_code)} · ${escapeHTML(item.profile_name)} · itens ${item.totals?.itemCount || 0} · tributos ${accountingCurrency(item.totals?.taxesCents)} · ${escapeHTML(item.status)}</small><em>Emissão bloqueada</em></div>`).join("")}</div>` : '<p class="fiscal-restricted">Ainda não há rascunhos fiscais integrados a vendas.</p>';
  ...
  fiscal.bind = function bindFiscalIntegration({ readiness, foundation, mappings, categories, taxSetup, drafts, branches, abilities, escapeHTML, dateBR, api, toast, reload }) {
    ...
    fiscalDraftForm?.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        output.innerHTML = `${taxPreviewHTML(result.calculation, escapeHTML)}<p class="fiscal-draft-success">Rascunho #${result.draftId} gravado. Emissão continua bloqueada.</p>`;
