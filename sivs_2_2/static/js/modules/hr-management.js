(function initializeHR(global) {
  ...
  function timePanel(data) {
    ...
    return `<section class="panel hr-panel" data-hr-section="time">
      <div class="panel-head"><div><h3>Ponto eletrônico</h3><small class="muted">Importe o arquivo oficial do relógio (AFD 004) ou uma planilha CSV. As marcações originais ficam preservadas para conferência.</small></div><span class="status">${data.imports.length} lote(s)</span></div>
      <div class="panel-body">
        ...
        <div class="hr-action-grid">
          ${canImport ? `<form id="hrTimeImportForm" class="hr-compact-form"><h4>Importar marcações do relógio</h4><label class="field"><span>Unidade *</span><select name="branchId" required>${data.branches.map((item) => option(item)).join("")}</select></label><label class="field"><span>Formato do arquivo</span><select name="format"><option value="AUTO">Detectar automaticamente</option><option value="AFD">AFD 004 (arquivo fiscal do relógio)</option><option value="CSV">CSV (planilha)</option></select></label><label class="field"><span>Tipo de relógio (REP) *</span><select name="repType" required><option value="1">REP-C (equipamento físico)</option><option value="2">REP-A (alternativo)</option><option value="3">REP-P (programa)</option></select><small>REP é o registro eletrônico de ponto.</small></label><label class="field full"><span>Arquivo do relógio *</span><input name="file" type="file" accept=".txt,.afd,.csv,text/plain,text/csv" required></label><button class="primary" type="submit">Verificar e importar</button><p id="hrTimeImportError" class="form-error hidden full" role="alert"></p></form>` : ""}
        ...
        <div class="hr-export-actions">${data.abilities.export_hr ? `<a class="secondary" href="/api/hr/time/export?period=${data.period}&format=csv">Exportar horas em CSV</a><a class="secondary" href="/api/hr/time/export?period=${data.period}&format=aej">Gerar arquivo AEJ para validação</a>` : ""}<small>AEJ é o arquivo de jornada. Esta exportação ainda precisa da assinatura P7S do desenvolvedor do PTRP para uso fiscal. Marcações importadas: ${data.imports.reduce((sum, item) => sum + item.imported_count, 0)} · sem vínculo: ${data.imports.reduce((sum, item) => sum + item.unmatched_count, 0)}</small></div>
      </div></section>`;
  }
  ...
  function payrollPanel(data) {
    ...
    const employmentOptions = data.employments.map((item) => option(item)).join("");
    const rubricOptions = data.rubrics.map((item) => `<option value="${item.id}">${hr.escape(item.code)} · ${hr.escape(item.description)}</option>`).join("");
    ...
    return `<section class="panel hr-panel" data-hr-section="payroll">
      ...
      <div class="panel-body">
        ...
        ${data.abilities.process_payroll && run?.status !== "CLOSED" ? `<div class="hr-action-grid"><form id="hrPayrollEventForm" class="hr-compact-form"><h4>Evento por rubrica revisada</h4><label class="field full"><span>Vínculo *</span><select name="employmentId" required><option value="">Selecione</option>${employmentOptions}</select></label><label class="field full"><span>Rubrica vigente *</span><select name="rubricId" required><option value="">Selecione</option>${rubricOptions}</select><small>Natureza e incidências vêm da versão revisada.</small></label><label class="field"><span>Valor *</span><input name="amount" inputmode="decimal" required></label><label class="field full"><span>Justificativa *</span><textarea name="reason" minlength="10" maxlength="500" required></textarea></label><button class="secondary" type="submit" ${rubricOptions ? "" : "disabled"}>Adicionar evento</button><p id="hrPayrollEventError" class="form-error hidden full" role="alert"></p></form><div class="hr-payroll-actions"><button class="primary" type="button" id="hrCalculatePayroll">Calcular/recalcular prévia</button><small>A competência só fecha se jornada e eventos estiverem consistentes e governados.</small>${run && data.abilities.close_payroll ? `<form id="hrClosePayrollForm"><label class="field"><span>Justificativa do fechamento *</span><textarea name="reason" minlength="10" required></textarea></label><button class="primary" type="submit">Fechar folha revisionada</button></form>` : ""}</div></div>` : ""}
  ...
  function compliancePanel(data) {
    const state = data.compliance;
    const scoreLabels = { cadastros: "Cadastros", jornada: "Jornada", folha: "Folha", conformidade: "Conformidade" };
    const scores = Object.entries(state.scores).map(([key, value]) => `<article class="${value < 90 ? "warning" : ""}"><span>${scoreLabels[key]}</span><strong>${value}/100</strong></article>`).join("");
    const notices = [...state.blockers.map((item) => `Bloqueio: ${item.message}`), ...state.warnings.map((item) => `Revisar: ${item.message}`)];
    const rubrics = data.rubrics.map((item) => `<tr><td><strong>${hr.escape(item.code)}</strong><small>${hr.escape(item.description)}</small></td><td>${hr.escape(item.esocial_nature_code)}</td><td>${hr.escape(item.cod_inc_cp)} / ${hr.escape(item.cod_inc_irrf)} / ${hr.escape(item.cod_inc_fgts)}</td><td>${hr.escape(item.reviewer_name)}</td></tr>`).join("");
    return `<section class="panel hr-panel" data-hr-section="compliance"><div class="panel-head"><div><h3>Prontidão trabalhista</h3><small class="muted">Vigências e evidências revisadas, sem presumir enquadramento.</small></div><span class="status ${state.overall >= 90 ? "concluído" : "pendente"}">${state.overall}/100</span></div><div class="panel-body"><div class="hr-summary hr-readiness">${scores}</div><div class="hr-readiness-list" role="status" aria-live="polite">${notices.map((message) => `<p>${hr.escape(message)}</p>`).join("") || "<p>Sem pendências identificadas nesta competência.</p>"}</div><div class="table-wrap borderless"><table class="data-table"><thead><tr><th>Rubrica</th><th>Natureza</th><th>CP / IRRF / FGTS</th><th>Revisão</th></tr></thead><tbody>${rubrics || '<tr><td colspan="4">Cadastre rubricas antes de lançar eventos variáveis.</td></tr>'}</tbody></table></div>${data.abilities.manage_hr_compliance ? `<div class="hr-action-grid"><form id="hrRubricForm" class="hr-compact-form"><h4>Nova versão de rubrica</h4><label class="field"><span>Código *</span><input name="code" maxlength="20" required></label><label class="field"><span>Tipo *</span><select name="kind"><option value="EARNING">Provento</option><option value="DEDUCTION">Desconto</option></select></label><label class="field full"><span>Descrição *</span><input name="description" required></label><label class="field"><span>Início *</span><input name="effectiveFrom" type="date" required></label><label class="field"><span>Fim</span><input name="effectiveUntil" type="date"></label><label class="field"><span>Natureza eSocial *</span><input name="esocialNatureCode" pattern="[0-9]{4}" maxlength="4" required></label><label class="field"><span>Incidência CP *</span><input name="codIncCp" maxlength="8" required></label><label class="field"><span>Incidência IRRF *</span><input name="codIncIrrf" maxlength="8" required></label><label class="field"><span>Incidência FGTS *</span><input name="codIncFgts" maxlength="8" required></label><label class="field"><span>Efeito INSS</span><select name="inssEffect"><option value="BASE">Compõe base</option><option value="NONE">Não compõe</option></select></label><label class="field"><span>Efeito IRRF</span><select name="irrfEffect"><option value="BASE">Compõe base</option><option value="DEDUCT">Deduz da base</option><option value="NONE">Sem efeito</option></select></label><label class="field"><span>Efeito FGTS</span><select name="fgtsEffect"><option value="BASE">Compõe base</option><option value="NONE">Não compõe</option></select></label><label class="field full"><span>Fonte oficial *</span><input name="normativeSourceUrl" type="url" placeholder="https://www.gov.br/..." required></label><label class="field full"><span>Análise do responsável *</span><textarea name="reviewNote" minlength="20" required></textarea></label><button class="primary" type="submit">Registrar versão imutável</button><p class="form-error hidden full" role="alert"></p></form><form id="hrComplianceForm" class="hr-compact-form"><h4>Evidência de conformidade</h4><label class="field full"><span>Tema *</span><select name="kind"><option value="COLLECTIVE_INSTRUMENT">Convenção/acordo coletivo</option><option value="ESOCIAL_REGISTRATION">Cadastro eSocial</option><option value="SST_PROGRAM">Programa de SST</option><option value="OCCUPATIONAL_HEALTH">Saúde ocupacional</option></select></label><label class="field"><span>Situação *</span><select name="status"><option value="REVIEWED">Revisado</option><option value="NOT_APPLICABLE">Não aplicável</option></select></label><label class="field"><span>Identificador</span><input name="identifier"></label><label class="field full"><span>Título *</span><input name="title" minlength="5" required></label><label class="field"><span>Início *</span><input name="effectiveFrom" type="date" required></label><label class="field"><span>Fim</span><input name="effectiveUntil" type="date"></label><label class="field full"><span>Evidência oficial</span><input name="evidenceUrl" type="url" placeholder="https://www.gov.br/..."></label><label class="field full"><span>Análise / justificativa *</span><textarea name="reviewNote" minlength="20" required></textarea></label><button class="primary" type="submit">Registrar evidência imutável</button><p class="form-error hidden full" role="alert"></p></form></div>` : ""}<p class="compliance-note compact">Não há transmissão automática ao eSocial, FGTS Digital ou DCTFWeb; a tela prepara e prova a conferência humana.</p></div></section>`;
  }

  function lifecyclePanel(data) {
    const rows = data.lifecycleEvents.map((item) => `<tr><td>${hr.escape(item.event_date)}</td><td><strong>${hr.escape(item.employee_name)}</strong></td><td>${hr.escape(item.event_type)}</td><td>${hr.escape(item.esocial_event_code)}</td><td>${item.legal_deadline ? hr.escape(item.legal_deadline) : "—"}</td></tr>`).join("");
    const employmentOptions = data.employments.map((item) => option(item)).join("");
    return `<section class="panel hr-panel" data-hr-section="lifecycle"><div class="panel-head"><div><h3>Eventos trabalhistas</h3><small class="muted">Férias, afastamentos, alterações, desligamento, 13º e SST com referência eSocial.</small></div><span class="status">${data.lifecycleEvents.length}</span></div><div class="panel-body"><div class="table-wrap borderless"><table class="data-table"><thead><tr><th>Data</th><th>Colaborador</th><th>Evento</th><th>eSocial</th><th>Prazo</th></tr></thead><tbody>${rows || '<tr><td colspan="5">Nenhum evento nesta competência.</td></tr>'}</tbody></table></div>${data.abilities.manage_hr_employments ? `<form id="hrLifecycleForm" class="hr-form-grid"><label class="field full"><span>Vínculo *</span><select name="employmentId" required><option value="">Selecione</option>${employmentOptions}</select></label><label class="field"><span>Evento *</span><select name="eventType"><option value="CONTRACT_CHANGE">Alteração contratual</option><option value="LEAVE">Afastamento</option><option value="VACATION">Férias</option><option value="TERMINATION">Desligamento</option><option value="THIRTEENTH">13º salário</option><option value="SST_CAT">CAT / S-2210</option><option value="SST_ASO">ASO / S-2220</option><option value="SST_EXPOSURE">Exposição / S-2240</option></select></label><label class="field"><span>Data inicial *</span><input name="eventDate" type="date" required></label><label class="field"><span>Data final</span><input name="endDate" type="date"></label><label class="field"><span>Código do motivo / evidência</span><input name="officialReasonCode" maxlength="80"></label><fieldset class="full hr-vacation-fields"><legend>Somente para férias</legend><div class="hr-form-grid"><label class="field"><span>Início aquisitivo</span><input name="acquisitionStart" type="date"></label><label class="field"><span>Fim aquisitivo</span><input name="acquisitionEnd" type="date"></label><label class="field"><span>Data do aviso</span><input name="noticeDate" type="date"></label><label class="field"><span>Data do pagamento</span><input name="paymentDate" type="date"></label></div></fieldset><label class="field full"><span>Conferência / justificativa *</span><textarea name="reason" minlength="20" required></textarea></label><button class="primary" type="submit">Registrar evento revisado</button><p class="form-error hidden full" role="alert"></p></form>` : ""}</div></section>`;
  }
  ...
  function render(data) {
    ...
    return `<section class="hr-hero"><div><p class="eyebrow gold">CENTRAL DE RH</p><h2>Pessoas, jornada e folha em um só lugar.</h2><p>As marcações do relógio são preservadas, a jornada pode ser conferida e a folha só fecha com as regras legais aplicáveis ao período.</p></div><label class="field hr-period"><span>Mês de referência</span><input id="hrPeriod" type="month" value="${data.period}"></label></section>
      ...
      <nav class="hr-jump" aria-label="Áreas do RH"><a href="#" data-hr-jump="people">Vínculos</a><a href="#" data-hr-jump="time">Ponto e horas</a><a href="#" data-hr-jump="payroll">Folha</a><a href="#" data-hr-jump="lifecycle">Eventos</a><a href="#" data-hr-jump="compliance">Prontidão</a></nav>
      ${employmentPanel(data)}${timePanel(data)}${payrollPanel(data)}${lifecyclePanel(data)}${compliancePanel(data)}`;
  }
  ...
  function bind(data) {
    ...
    eventForm?.addEventListener("submit", (event) => { event.preventDefault(); submit(eventForm, async (values) => { await hr.api("/api/hr/payroll/events", { method: "POST", body: JSON.stringify({ period: data.period, employmentId: Number(values.get("employmentId")), rubricId: Number(values.get("rubricId")), amount: values.get("amount"), reason: values.get("reason") }) }); hr.toast("Evento governado adicionado; recalcule a prévia."); await hr.load(hr.context); }); });
    document.querySelector("#hrCalculatePayroll")?.addEventListener("click", async (event) => { event.currentTarget.disabled = true; try { const result = await hr.api("/api/hr/payroll/preview", { method: "POST", body: JSON.stringify({ period: data.period }) }); hr.toast(result.readyToClose ? "Prévia calculada e pronta para conferência." : "Prévia calculada com pendências de jornada ou rubricas."); await hr.load(hr.context); } catch (error) { hr.toast(error.message); } finally { event.currentTarget.disabled = false; } });
    const closeForm = document.querySelector("#hrClosePayrollForm");
    closeForm?.addEventListener("submit", (event) => { event.preventDefault(); submit(closeForm, async (values) => { await hr.api("/api/hr/payroll/close", { method: "POST", body: JSON.stringify({ period: data.period, revision: data.payroll[0].revision, reason: values.get("reason") }) }); hr.toast("Folha fechada. Para corrigir depois, será necessário um procedimento formal de reabertura."); await hr.load(hr.context); }); });
    const rubricForm = document.querySelector("#hrRubricForm");
    rubricForm?.addEventListener("submit", (event) => { event.preventDefault(); submit(rubricForm, async (values) => { await hr.api("/api/hr/payroll/rubrics", { method: "POST", body: JSON.stringify(Object.fromEntries(values)) }); hr.toast("Versão da rubrica registrada e auditada."); await hr.load(hr.context); }); });
    const complianceForm = document.querySelector("#hrComplianceForm");
    complianceForm?.addEventListener("submit", (event) => { event.preventDefault(); submit(complianceForm, async (values) => { await hr.api("/api/hr/compliance-records", { method: "POST", body: JSON.stringify(Object.fromEntries(values)) }); hr.toast("Evidência de conformidade registrada."); await hr.load(hr.context); }); });
    const lifecycleForm = document.querySelector("#hrLifecycleForm");
    lifecycleForm?.addEventListener("submit", (event) => { event.preventDefault(); submit(lifecycleForm, async (values) => { const raw = Object.fromEntries(values); const detail = { officialReasonCode: raw.officialReasonCode, acquisitionStart: raw.acquisitionStart, acquisitionEnd: raw.acquisitionEnd, noticeDate: raw.noticeDate, paymentDate: raw.paymentDate }; await hr.api("/api/hr/lifecycle-events", { method: "POST", body: JSON.stringify({ employmentId: Number(raw.employmentId), eventType: raw.eventType, eventDate: raw.eventDate, endDate: raw.endDate, reason: raw.reason, detail }) }); hr.toast("Evento trabalhista registrado e auditado."); await hr.load(hr.context); }); });
  }
